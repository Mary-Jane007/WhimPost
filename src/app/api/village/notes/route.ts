...
export async function POST(req: NextRequest) {
  ...
  if (text.length < 2) return jsonError("Write a little something for the board");

  const db = getDb();
  const id = uuidv4();
  db.prepare(
    `INSERT INTO village_notes (id, village_id, author_id, body, anonymous)
     VALUES (?, ?, ?, ?, ?)`
  ).run(id, user.villageId, user.id, text, anonymous ? 1 : 0);
